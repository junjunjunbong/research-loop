...
| T4 | Benchmark contamination / data leakage: sources reveal test answers or tuned-on-benchmark tricks | `prohibited_interpretations` on claim records; retrieval guidance forbids queries for benchmark answers or test-set contents; optional `retrieval_cutoff` date in `knowledge_access`; evaluation compatibility checks remain authoritative | PR5, PR7 |
...
| T6 | Reproducibility loss: source content changes after the fact | Immutable `revision` and/or `content_sha256` required at registration; packs are content-addressed with `SHA256SUMS` (mirrors `vendor/` practice); agent-retrieved sources snapshot the fetched content locally (campaign artifact, never redistributed) before normalization | PR4, PR5, PR7 |
...
| T8 | Scope creep: retrieval quietly adds network access to a local-only loop | `policy.knowledge_access.allow_network` (distinct from `allow_remote`) defaults false and is approval-bound: `mode: agent_retrieval` must be visible in the approved dry run. Retrieval is performed agent-side only; the runner never gains network code | PR5, PR7 |
